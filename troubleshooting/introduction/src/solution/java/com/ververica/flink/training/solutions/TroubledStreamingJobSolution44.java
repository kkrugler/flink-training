...
/**
 * Troubled streaming job evolved on top of {@link TroubledStreamingJobSolution43} by using a Tuple2
 * to save the state in the aggregator.
 */
public class TroubledStreamingJobSolution44 {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (!parameters.has("discard") && isLocal(parameters)) {
        ...
        env.execute(TroubledStreamingJobSolution44.class.getSimpleName());
    }
    ...
    public static class MeasurementWindowAggregatingFunction
            implements AggregateFunction<
                    SimpleMeasurement, Tuple2<Long, Double>, Tuple2<Long, Double>> {
        ...
        @Override
        public Tuple2<Long, Double> add(SimpleMeasurement record, Tuple2<Long, Double> aggregate) {
            aggregate.f0 += 1;
            aggregate.f1 += record.getValue();
            return aggregate;
        }

        @Override
        public Tuple2<Long, Double> getResult(Tuple2<Long, Double> windowedMeasurements) {
            return windowedMeasurements;
        }

        @Override
        public Tuple2<Long, Double> merge(
                final Tuple2<Long, Double> agg1, final Tuple2<Long, Double> agg2) {
            agg2.f0 += agg1.f0;
            agg2.f1 += agg1.f1;
            return agg2;
        }
    ...
    public static class MeasurementWindowProcessFunction
            ...
            Tuple2<Long, Double>, WindowedMeasurements, String, TimeWindow> {
        ...
        public void process(
            ...
            // Windows with pre-aggregation only forward the final aggregation
            // to the WindowFunction
            Tuple2<Long, Double> aggregate = input.iterator().next();

            final TimeWindow window = context.window();
            WindowedMeasurements result = new WindowedMeasurements();
            result.setWindow(window);
            result.setLocation(location);
            result.setEventsPerWindow(aggregate.f0);
            result.setSumPerWindow(aggregate.f1);
