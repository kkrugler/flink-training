...
public abstract class BaseShoppingCartGenerator<T extends BaseShoppingCartRecord> implements SerializableFunction<Long, T> {
    ...
    private List<T> activeCarts = new ArrayList<>();

    public BaseShoppingCartGenerator(long startingTime) {
        this.curTime = startingTime;

    }

    public abstract T createShoppingCartRecord();

    @Override
    public T apply(Long recordIndex) {
    ...
    private void updateTransactionTime(T result) {
    ...
    @VisibleForTesting
    public T createShoppingCart() {
        T result = createShoppingCartRecord();
