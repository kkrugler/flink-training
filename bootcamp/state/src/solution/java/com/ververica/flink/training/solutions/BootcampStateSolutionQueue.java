...
public class BootcampStateSolutionQueue<T> extends BootcampStateQueue<T> {
    ...
    public void open(RuntimeContext ctx) {
        headState =
                ctx.getState(new ValueStateDescriptor<>(statePrefix + "queue-head", Long.class));
        tailState =
                ctx.getState(new ValueStateDescriptor<>(statePrefix + "queue-tail", Long.class));
        queueState =
                ctx.getMapState(
                        new MapStateDescriptor<>(statePrefix + "queue", Long.class, queueClass));
    }
