...
/**
 * Solution to the exercises in the eCommerce serialization lab, in increasing order of
 * complexity...
 *
 * <p>1. We strip down the incoming records and convert to a TrimmedShoppingCartRecord 2.
 * TrimmedShoppingCartRecord is serializable as a POJO 3. Use KeyedProcessFunction to find duration,
 * versus session window 4. Use simple structure for top two aggregation, versus priority queue
 */
public class BootcampSerializationSolutionWorkflow extends BootcampSerializationWorkflow {
    private static final Logger LOGGER =
            LoggerFactory.getLogger(BootcampSerializationSolutionWorkflow.class);
    ...
    public void build() {
        ...
        DataStream<TrimmedShoppingCart> watermarkedStream =
                cartStream
                        // Convert to a smaller/better version of ShoppingCartRecord.
                        .map(r -> new TrimmedShoppingCart(r))
                        .assignTimestampsAndWatermarks(
                                WatermarkStrategy.<TrimmedShoppingCart>forBoundedOutOfOrderness(
                                                Duration.ofMinutes(1))
                                        .withTimestampAssigner(
                                                (element, timestamp) ->
                                                        element.getTransactionTime()));

        DataStream<KeyedWindowResult> oneMinuteStream =
                watermarkedStream
        ...
        oneMinuteStream.sinkTo(oneMinuteSink);

        // Calculate the 5-minute window counts, using the 1-minute window count result as output.
        // We know
        // that the event time for records in the oneMinuteStream will be set by Flink to be the end
        // of
        // the window, so we can do a windowAll (which means a single task, no parallelism) to do a
        // second
        ...
        watermarkedStream
                ...
                .windowAll(
                        TumblingEventTimeWindows.of(Duration.ofMinutes(transactionWindowInMinutes)))
    ...
    private static class CountCartItemsAggregator
            implements AggregateFunction<TrimmedShoppingCart, Long, Long> {
        @Override
    ...
    private static class SetTimeFunction
            extends ProcessAllWindowFunction<Long, WindowAllResult, TimeWindow> {

        @Override
        public void process(Context ctx, Iterable<Long> elements, Collector<WindowAllResult> out)
                throws Exception {
    ...
    private static class OneMinuteWindowCountAggregator
            implements AggregateFunction<KeyedWindowResult, Long, Long> {
        @Override
        public Long createAccumulator() {
            return 0L;
        }
    ...
    private static class SetKeyAndTimeFunction
            extends ProcessWindowFunction<Long, KeyedWindowResult, String, TimeWindow> {
        @Override
        public void process(
                String key, Context ctx, Iterable<Long> elements, Collector<KeyedWindowResult> out)
                throws Exception {
            out.collect(
                    new KeyedWindowResult(
                            key, ctx.window().getStart(), elements.iterator().next()));
        }
    }
    ...
    private static class FindTransactionBoundsFunction
            extends KeyedProcessFunction<String, TrimmedShoppingCart, Tuple2<String, Long>> {
        ...
        public void open(OpenContext openContext) throws Exception {
            earliestTime =
                    getRuntimeContext()
                            .getState(new ValueStateDescriptor<>("earliestTime", Long.class));
            timerTime =
                    getRuntimeContext()
                            .getState(new ValueStateDescriptor<>("timerTime", Long.class));
            endTime =
                    getRuntimeContext().getState(new ValueStateDescriptor<>("endTime", Long.class));
        }

        @Override
        public void processElement(
                TrimmedShoppingCart in, Context ctx, Collector<Tuple2<String, Long>> out)
                throws Exception {
            ...
            // We assume properly ordered data, in that the completed transaction's time will always
            // be >= any
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<Tuple2<String, Long>> out)
                throws Exception {
            // Timer fired. If we have an end transaction, then we have a duration. If there's no
            // start, assume it's
            // a single action. If we don't have an end transaction, assume it's an abandoned cart
            // (do nothing).
            ...
            if (end != null) {
                ...
                // If we just call out.collect(), the record's event time is set to the timer's
                // timestamp. But
                // that's not what we want, as this timer is for when we end a session, so it's some
                // time
                // after the end of the session. We need to do the funky cast of the collector to a
    ...
    private static class SetDurationAndTimeFunction
            extends ProcessWindowFunction<
                    Tuple2<Long, Long>, KeyedWindowResult, String, TimeWindow> {
        @Override
        public void process(
                String key,
                Context ctx,
                Iterable<Tuple2<Long, Long>> elements,
                Collector<KeyedWindowResult> out)
                throws Exception {
            Tuple2<Long, Long> interval = elements.iterator().next();

            // If we didn't get a start (probably because the session window timed out, so all we
            // got is
            ...
            out.collect(
                    new KeyedWindowResult(key, ctx.window().getStart(), interval.f1 - interval.f0));
        }
    }
    ...
    private static class FindLongestTransactions
            implements AggregateFunction<
                    Tuple2<String, Long>, LongestTwoTransactions, List<Tuple2<String, Long>>> {
        @Override
    ...
    public static class LongestTwoTransactions {
        ...
        /**
         * Gets the first Tuple2.
         * @return The first Tuple2.
         */
        ...
        /**
         * Sets the first Tuple2.
         * @param first The first Tuple2 to set.
         */
        ...
        /**
         * Gets the second Tuple2.
         * @return The second Tuple2.
         */
        ...
        /**
         * Sets the second Tuple2.
         * @param second The second Tuple2 to set.
         */
        ...
        /**
         * Update our state variable with a new transaction, which
         * might be one of the two longest (otherwise do nothing)
         *
    ...
    private static class EmitLongestTransactions
            extends ProcessAllWindowFunction<
                    List<Tuple2<String, Long>>, KeyedWindowResult, TimeWindow> {
